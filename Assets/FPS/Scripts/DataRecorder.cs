...
public static class DataRecorder {
    ...
    public static bool recordLevelFinishTime(float _Time)
    {
        ...
        Debug.Log(filePath);
        string lineToAdd = _Time.ToString();

        using (StreamWriter sw = File.AppendText(filePath + "_Time.txt")) //This line will try to open the file and if it doesn't exist, it will make it!
        {
            //Write death position vector to our text file as a new line
            sw.WriteLine(lineToAdd);
            sw.Close();
        ...
        ////Re-import the file to update the reference in the editor
        //AssetDatabase.ImportAsset(filePath+".txt");
        TextAsset asset = Resources.Load<TextAsset>(filePath + "_Time.txt");
